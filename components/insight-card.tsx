      ...
      <CardContent className="p-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <div className="mr-3 p-2 bg-purple-50 rounded-lg">{insight.icon}</div>
            <div>
              <div className="flex items-center">
                <h3 className="font-medium text-gray-900">{insight.title}</h3>
                <Badge className="ml-2 bg-purple-100 text-purple-800">AI Insight</Badge>
              </div>
              <p className="text-xs text-gray-500 mt-0.5">{insight.description}</p>
              <div className="flex items-center mt-1">
                {icon}
                <span className={`text-xs ml-1 ${trendColor}`}>{insight.trendText}</span>
              </div>
            </div>
          </div>
